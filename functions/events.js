...
module.exports = {
    ...
    teamJoin: function (user, res) {
        ...
        if (user.is_bot === false) {
            ...
            util.validateTeamAccess(id, res, hasAccess => {
                bot.onboardMsg(user, teamID, res);
            });
        } else {
            res.status(OK).send();
        }
                ...
                if (deleted) {
                    ...
                    .then(snapshot => {
                        if (snapshot.val() && Object.keys(snapshot.val())[0]) {
                            ...
                            return database.ref('workspaces/'+workspaceId+'/users/').orderByChild('user_id').equalTo(user.id)
                            .transaction(userJson => {
                                    ...
                                    if (userJson.active) {
                                        ...
                                        for (var tag in userTags) {
                                            ...
                                            .then(snapshot => {
                                                if (snapshot.val() && Object.keys(snapshot.val())[0]) {
                                                    var workspaceId = Object.keys(snapshot.val())[0];
                                                    return database.ref('tags/'+workspaceId+'/tags/').orderByChild('tag_title').equalTo(tag)
    ...
    enterpriseMigration: function (teamID, enterpriseID) {
        ...
        .then(snapshot => {
            if (snapshot.val() && Object.keys(snapshot.val())[0]) {
                var workspaceId = Object.keys(snapshot.val())[0];
                database.ref('installations/' + workspaceId).child('bot_token').once('value').then(snapshot => {
                    var token = snapshot.val();
                    database.ref('workspaces').orderByChild('team').equalTo(teamID).once('value')
                    .then(snapshot => {
                        if (snapshot.val() && Object.keys(snapshot.val())[0]) {
                            var workspaceId = Object.keys(snapshot.val())[0];
                            database.ref('workspaces/' + workspaceId + '/users').once('value')
                            .then(snapshot => {
                                ...
                                request.get('https://slack.com/api/migration.exchange?token=' + token + '&users=' + userRequestString, (err, res, body) => {
                                    if (err) {
                                        ...
                                        // Update user index.
                                        return database.ref('workspaces/' + workspaceId + '/users').update(update);
                                    }
                                });
                                return null;
                            }).catch(err => {
        ...
        database.ref('feedback').orderByChild('team').equalTo(teamID).once('value')
        .then(snapshot => {
            if (snapshot.val() && Object.keys(snapshot.val())[0]) {
                var workspaceId = Object.keys(snapshot.val())[0];
                var data = Object.values(snapshot.val())[0];
                data[teamID] = enterpriseID;
                return database.ref('feedback'+workspaceId).update(data);
            }
            return;
        }).catch(err => {
            console.log(err);
        ...
        database.ref('tags').orderByChild('team').equalTo(teamID).once('value')
        .then(snapshot => {
            if (snapshot.val() && Object.keys(snapshot.val())[0]) {
                var workspaceId = Object.keys(snapshot.val())[0];
                var data = Object.values(snapshot.val())[0];
                data[teamID] = enterpriseID;
                return database.ref('tags'+workspaceId).update(data);
            }
            return;
        }).catch(err => {
            console.log(err);
        ...
        .then(snapshot => {
            if (snapshot.val() && Object.keys(snapshot.val())[0]) {
                ...
                var data = Object.values(snapshot.val())[0];
                data[teamID] = enterpriseID;
                return database.ref('workspaces'+workspaceId).update(data);
            }
            return;
        }).catch(err => {
            console.log(err);
        ...
        .then(snapshot => {
            if (snapshot.val() && Object.keys(snapshot.val())[0]) {
                ...
                return database.ref('installations'+workspaceId).update(data);
            }
            return;
        }).catch(err => {
            console.log(err);
