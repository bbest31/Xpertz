...
const OK = 200;
const presetTagList = require('./preset_tags_dev');
...
module.exports = {
    ...
    presetTagOptions: function (teamId) {
        ...
        database.ref('installations/' + teamId).once('value').then(snapshot => {
            ...
            bot_token = snapshot.val().bot_token;
                ...
                if (err) {
                    ...
                    members.forEach(user => {
                        if (user.is_owner) {
                            let userId = user.id;
                            //Get DM id
                            request.get('https://slack.com/api/conversations.open?token=' + bot_token + '&users=' + userId, (err, res, body) => {
                                if (err) {
                                    return console.log(err);
                                } else {
                                    let payload = JSON.parse(body);
